...
class DiffusionUnit(nn.Module):
    def __init__(self, d_in, nsample):
        super().__init__()
    ...
    def forward(self, pxo):
        ...
        u_tt = self.varphi(self.phi(nabla_u).mean(1)) + u_t 
        return u_tt
...
class DiffusionUnit(nn.Module):
    ...
    def __init__(self, d_in, d_out, share_planes=None, nsample=16):
        super().__init__()
        print(f'\n Diffusion Unit: {d_in} \n')
        self.nsample = nsample
        self.phi = nn.Sequential(
...
def DUNet_seg(**kwargs):
    model = PointTransformerSeg(DiffusionUnit, [2, 3, 4, 6, 3], **kwargs)
    return model
