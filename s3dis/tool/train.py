...
from util.common_util import AverageMeter, intersectionAndUnionGPU, find_free_port, CrossEntropyWithSmoothing
from util.data_util import collate_fn
from util import transform as t


def get_parser():
    parser = argparse.ArgumentParser(description='Diffusion Unit')
    parser.add_argument('--config', type=str, default='config/s3dis/res.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/res.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main():
    ...
    if args.manual_seed is None: 
        args.manual_seed = np.random.randint(10000, size=1)[0] 
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'scene_seg_net':
        from model.architectures import SceneSegNet as Model
    else:
    ...
    criterion = CrossEntropyWithSmoothing(label_smoothing=args.label_smoothing, ignore_index=args.ignore_label)
    ...
    transform_list = []
    if 'cont' in args.aug:
        transform_list.append(t.ChromaticAutoContrast())
    if 'ctrans' in args.aug:
        transform_list.append(t.ChromaticTranslation())
    if 'cjit' in args.aug:
        transform_list.append(t.ChromaticJitter())
    if 'hstrans' in args.aug:
        transform_list.append(t.HueSaturationTranslation())
    if 'rot' in args.aug:
        transform_list.append(t.RandomRotate())
    if 'scale' in args.aug:
        transform_list.append(t.RandomScale([0.66, 1.5], anisotropic=True))
    if 'jit' in args.aug:
        transform_list.append(t.RandomJitter())
    if 'cdrop' in args.aug:
        transform_list.append(t.RandomDropColor())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            torch.save({'epoch': epoch_log, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),
                        'scheduler': scheduler.state_dict(), 'best_iou': best_iou, 'is_best': is_best, 'seed': args.manual_seed}, filename)
            if is_best:
                logger.info('Best validation mIoU updated to: {:.4f}'.format(best_iou))
                shutil.copyfile(filename, args.save_path + '/model/model_best.pth')

        if main_process():
            logger.info('Current Best mIoU:  {:.4f}'.format(best_iou))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        intersection, union, target = intersection.detach().cpu().numpy(), union.detach().cpu().numpy(), target.detach().cpu().numpy()
