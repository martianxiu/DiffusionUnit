...
def prepare_input(coord, feat):
    coord_mean = np.mean(coord, 0)
    coord -= coord_mean
    feat = feat / 255.
    feat = np.hstack([feat, coord[:, 2:]])
    return coord, feat
...
def test(model, criterion, names):
    ...
    for idx, item in enumerate(data_list):
        ...
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            ...
            pred, label = np.load(pred_save_path, allow_pickle=True), np.load(label_save_path, allow_pickle=True)
        else:
            ...
            for i in range(idx_size):
                ...
                if args.voxel_max and coord_part.shape[0] > args.voxel_max:
                    ...
                    coord_part, feat_part = prepare_input(coord_part, feat_part)
        ...
        intersection, union, target = intersectionAndUnion(pred, label, args.num_classes, args.ignore_label)
    ...
    # calculation 2
    intersection, union, target = intersectionAndUnion(np.concatenate(pred_save), np.concatenate(label_save), args.num_classes, args.ignore_label)
